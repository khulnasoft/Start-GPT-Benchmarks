...
from startbenchmarks.reports.reports import (
    finalize_reports,
    generate_combined_suite_report,
    generate_single_call_report,
    session_finish,
)
from startbenchmarks.utils.data_types import SuiteConfig
from startbenchmarks.generate_test import ALL_REPORT_DATA

GLOBAL_TIMEOUT = (
    1500  # The tests will stop after 25 minutes so we can send the reports.
)

pytest_plugins = ["startbenchmarks.utils.dependencies"]
...
def config(request: Any) -> None:
    print(f"Config file: {startbenchmarks.start_benchmark.CONFIG_PATH}")
...
def check_regression(request: Any) -> None:
    ...
    data = startbenchmarks.start_benchmark.get_regression_data()
...
def run_agent(request: Any) -> Any:
    ...
    if config.get("api_mode"):
        command = [sys.executable, "-m", "startbenchmarks.benchmarks"]
        process = subprocess.Popen(
            command,
            stdout=subprocess.PIPE,
            stderr=subprocess.STDOUT,
            universal_newlines=True,
            cwd=startbenchmarks.start_benchmark.HOME_DIRECTORY,
        )
        time.sleep(3)
        yield
