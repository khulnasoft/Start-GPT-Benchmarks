...
import startbenchmarks.start_benchmark
from startbenchmarks.utils.data_types import DIFFICULTY_MAP, DifficultyLevel, SuiteConfig
from startbenchmarks.utils.get_data_from_helicone import get_data_from_helicone
from startbenchmarks.utils.utils import (
    ...
    for i, test_name in enumerate(challenge_data["ground"]):
        ...
        if 1 in scores.get("scores_obj", {}).get(test_name, []):
            ...
            if not mock:  # don't remove if it's a mock test
                startbenchmarks.start_benchmark.REGRESSION_MANAGER.remove_test(test_name)
...
def get_previous_test_results(
    ...
    prev_test_results = startbenchmarks.start_benchmark.INTERNAL_INFO_MANAGER.tests.get(
        test_name, []
    )
    ...
    if not mock:
        ...
        startbenchmarks.start_benchmark.INTERNAL_INFO_MANAGER.add_test(
...
def update_regression_tests(
    ...
    if len(prev_test_results) >= 3 and prev_test_results[-3:] == [True, True, True]:
        ...
        startbenchmarks.start_benchmark.REGRESSION_MANAGER.add_test(test_name, test_details)
...
def finalize_reports(item: Any, challenge_data: dict[str, Any]) -> None:
    ...
    if info_details and test_name:
        ...
        startbenchmarks.start_benchmark.INFO_MANAGER.add_test(test_name, info_details)
...
def generate_separate_suite_reports(suite_reports: dict) -> None:
    for prefix, suite_file_datum in suite_reports.items():
        ...
        for name in suite_file_datum:
            ...
            try:
                test_data = startbenchmarks.start_benchmark.INFO_MANAGER.tests[
                    name
            ...
            startbenchmarks.start_benchmark.INFO_MANAGER.remove_test(name)
        ...
        startbenchmarks.start_benchmark.INFO_MANAGER.add_test(prefix, info_details)
...
def session_finish(suite_reports: dict) -> None:
    ...
    with open(startbenchmarks.start_benchmark.CONFIG_PATH, "r") as f:
        config = json.load(f)

    startbenchmarks.start_benchmark.INTERNAL_INFO_MANAGER.save()
    startbenchmarks.start_benchmark.INFO_MANAGER.end_info_report(config)
    startbenchmarks.start_benchmark.REGRESSION_MANAGER.save()
