...
from startbenchmarks.utils.data_types import DIFFICULTY_MAP, DifficultyLevel
...
def get_test_path(json_file: str | Path) -> str:
    ...
    # Find the index of "startbenchmarks" in the path parts
    try:
        startbenchmarks_index = json_file.parts.index("startbenchmarks")
    except ValueError:
        raise ValueError("Invalid challenge location.")

    # Create the path from "startbenchmarks" onwards
    challenge_location = Path(*json_file.parts[startbenchmarks_index:])
...
def calculate_dynamic_paths() -> tuple[Path, str, str, str, str, str]:
    ...
    if os.path.join("Start-GPT-Benchmarks", "backend") in str(
        HOME_DIRECTORY
    ):  # accounting for backend calls
        HOME_DIRECTORY = HOME_DIRECTORY.parent

    benchmarks_folder_path = HOME_DIRECTORY / "startbenchmarks"

    if AGENT_NAME and not os.path.join("Start-GPT-Benchmarks", "agent") in str(
        HOME_DIRECTORY
    ):
        ...
        # otherwise the default is when home is an agent (running startbenchmarks from agent/agent_repo)
...
def find_absolute_benchmark_path() -> Path:
    ...
    # Find the position of "Start-GPT-Benchmarks" in the path
    benchmark_path_index = (
        current_path.parts.index("Start-GPT-Benchmarks")
        if "Start-GPT-Benchmarks" in current_path.parts
        else None
    )

    if benchmark_path_index is not None:
        # Construct the absolute path starting from "Start-GPT-Benchmarks"
        benchmark_path = Path(*current_path.parts[: benchmark_path_index + 1])

        return benchmark_path
    else:
        raise ValueError(
            "The directory 'Start-GPT-Benchmarks' is not found in the current path."
        )
